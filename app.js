    ...
    $("#searchItem").on("click", function() {
        ...
        $.ajax({
            ...
            for (var i = 0; i < response.items.length; i++) {
                ...
                var produceDesc = $('<p>').html(response.items[i].shortDescription);
                descriptionDiv.append(produceDesc);

                // link for youtube
                descriptionDiv.append($("<a onclick='mymodal(\"" + (response.items[i].name).replace(/"/g, '\\"') + "," + i + "\")'>").html("<h4 class='youtube' id='youtube" + i + " '>find more with a youtube review, click here!</h4>"));
